...
void LCD_drawRowOfSquares(uint16_t startX, uint16_t startY, uint16_t squareSize, uint16_t gapSize, uint16_t numSquares, uint16_t color){
	int i =0;
	for(i=0; i<numSquares; ++i){
		 LCD_DrawLine(startX, startY, startX + squareSize, startY, color);  
     LCD_DrawLine(startX, startY, startX, startY + squareSize, color);  
     LCD_DrawLine(startX + squareSize, startY, startX + squareSize, startY + squareSize, color);  
     LCD_DrawLine(startX, startY + squareSize, startX + squareSize, startY + squareSize, color);  
     startX += squareSize + gapSize;  
	}
}

void drawAllSquares(uint16_t squareColor){
	
	 uint16_t squareSize = 24;
   uint16_t gapSize = 8;
	
	int row=0;
	for (row = 0; row < 7; ++row) {
        LCD_drawRowOfSquares(12, 12+row * (squareSize + gapSize), squareSize, gapSize, 7, squareColor);
    }
}

void drawRectangles(uint16_t startX, uint16_t startY, uint16_t rectWidth, uint16_t rectHeight, uint16_t gap) {
	
    int i;
    for (i = 0; i < 3; ++i) {
			
        uint16_t rectX = startX + i * (rectWidth + gap);
        uint16_t rectY = startY;

       
        LCD_DrawLine(rectX, rectY, rectX + rectWidth, rectY, White);           
        LCD_DrawLine(rectX, rectY, rectX, rectY + rectHeight, White);          
        LCD_DrawLine(rectX + rectWidth, rectY, rectX + rectWidth, rectY + rectHeight, White);  
        LCD_DrawLine(rectX, rectY + rectHeight, rectX + rectWidth, rectY + rectHeight, White);  
    }
}

void colorSquare(uint16_t startX, uint16_t startY, uint16_t color) {
	  uint16_t i =0;
		uint16_t j=0;

    for (i = 0; i < 23; ++i) {
        for (j = 0; j < 23; ++j) {
            LCD_SetPoint(startX + i, startY + j, color);
        }
    }
}

void colorHorizontalRectangle(uint16_t startX, uint16_t startY){
    ...
    for (i = 0; i < 57; ++i) {
        for (j = 0; j < 7; ++j) {
            LCD_SetPoint(startX + i, startY + j, Blue2);
        }
    }

}

void colorVerticalRectangle(uint16_t startX, uint16_t startY){
...
void cancelHorizontalRectangle(uint16_t startX, uint16_t startY){
    ...
    for (i = 0; i < 57; ++i) {
        for (j = 0; j < 7; ++j) {
            LCD_SetPoint(startX + i, startY + j, Black);
        }
    }

}

void cancelVerticalRectangle(uint16_t startX, uint16_t startY){
...
void initGraphic(void){
  ...
  LCD_Clear(Black);
	drawAllSquares(White);
	drawRectangles(4,260,73,50,6);
	
	GUI_Text(9, 265, (uint8_t *) " P1 Wall", White, Black);
	GUI_Text(104, 277, (uint8_t *) "   s", White, Black);
	GUI_Text(167, 265, (uint8_t *) " P2 Wall", White, Black);


}

void printVariable(uint16_t Xpos, uint16_t Ypos, int variable, uint16_t Color, uint16_t bkColor)
{
    char buffer[20];  
	
    sprintf(buffer, "%d", variable);  // from variable to a character string
	
    GUI_Text(Xpos, Ypos, buffer, Color, bkColor);  // Call  GUI_Text function
}
